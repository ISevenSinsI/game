...
<script>
	...
	$(".main_menu").on("click", ".travel_option", function(){
		if(one_time == false){
			...
			var interval=setInterval(function(){check_action_end(player_id);$.get("ajax/get_action_end",function(e){response=jQuery.parseJSON(e);if(response==="true"){stop_interval(interval);location_change=change_location(player_id,location_to_id,location_from_id)}})},3e3)
		}
	});
</script>